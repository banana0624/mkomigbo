...
// Preferred: middleware guard (if available)
if (is_file(PRIVATE_PATH . '/middleware/guard.php')) {
  if (!defined('REQUIRE_LOGIN')) {
    define('REQUIRE_LOGIN', true);
  }
  if (!defined('REQUIRE_PERMS')) {
    define('REQUIRE_PERMS', ['contributors.credits.delete', 'contributors.write']);
  }
  ...
  if (function_exists('require_permission')) {
    ...
    require_any_permission(['contributors.write', 'contributors.credits.delete']);
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // If user cancelled (no confirm), just redirect back to list
  header('Location: ' . url_for('/staff/contributors/credits/'));
  exit;
}
...
$body_class  = 'role--staff role--contrib credits-delete';
