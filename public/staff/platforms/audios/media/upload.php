<?php
// project-root/public/staff/platforms/audios/media/upload.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
  csrf_check();
  $dir = platform_uploads_dir($platform_slug);
  ...
  if ($f['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($f['tmp_name'], $dest)) {
      ...
      header('Location: ' . url_for("/staff/platforms/{$platform_slug}/media/"));
      exit;
    }
    flash('error','Failed to move uploaded file.');
  } else {
    flash('error','Upload error code: ' . (int)$f['error']);
  }
}

$page_title  = "{$platform_name} • Upload";
$active_nav  = 'staff';
...
$breadcrumbs = [
  ...
  ['label'=>'Media','url'=>"/staff/platforms/{$platform_slug}/media/"],
  ['label'=>'Upload'],
];
