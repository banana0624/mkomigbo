...
$platform_slug='forums'; $platform_name='Forums';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  csrf_check();
  $visibility=(string)($_POST['visibility'] ?? 'private');
  $banner=trim((string)($_POST['banner'] ?? ''));
  platform_settings_save($platform_slug, ['visibility'=>$visibility,'banner'=>$banner]);
  flash('success','Settings saved.'); header('Location: ' . url_for("/staff/platforms/{$platform_slug}/settings.php")); exit;
}
$settings = platform_settings_load($platform_slug);
$page_title="{$platform_name} • Settings"; $active_nav='staff'; $body_class="role--staff platform--{$platform_slug}";
$page_logo="/lib/images/platforms/{$platform_slug}.svg"; $stylesheets[]='/lib/css/ui.css';
$breadcrumbs=[['label'=>'Home','url'=>'/'],['label'=>'Staff','url'=>'/staff/'],['label'=>'Platforms','url'=>'/staff/platforms/'],['label'=>$platform_name,'url'=>"/staff/platforms/{$platform_slug}/"],['label'=>'Settings']];
...
<main class="container" style="max-width:720px;padding:1.25rem 0">
  ...
  <form method="post">
    ...
    <div class="field"><label>Default banner</label>
      <input class="input" type="text" name="banner" placeholder="/lib/images/banners/forums.jpg" value="<?= h($settings['banner'] ?? '') ?>">
    </div>
    <div class="actions">
