...
function main() {
  ...
  if (args.some(arg => arg.startsWith('--restore='))) return handleRestore(args);
  if (args.some(arg => arg.startsWith('--trash-expiry='))) return handleTrashExpiry(args);
  if (args.includes('--purge')) return handlePurge(args);
