...
/** Resolve absolute /public path once (no closures needed) */
...
$stylesheets  = array_values(array_unique(array_map(fn($p)=>'/'.ltrim((string)$p,'/'), $stylesheets)));
...
$scripts_head = array_values(array_unique(array_map(fn($p)=>'/'.ltrim((string)$p,'/'), $scripts_head)));

// Logo resolution
if (!empty($page_logo) && is_string($page_logo)) $page_logo = '/'.ltrim($page_logo, '/');
...
foreach ($logo_candidates as $cand) {
  if ($cand && asset_exists($cand)) { $logo_url = asset_url($cand); break; }
}
$logo_alt = isset($logo_alt) && $logo_alt !== '' ? $logo_alt : $site_name;

// Optional nav include (safe)
    ...
    <div class="container header-bar" style="display:flex;align-items:center;gap:.75rem;justify-content:space-between;padding:.6rem 0;">
      <div class="brand" style="display:flex;align-items:center;gap:.6rem;">
        <a class="logo" href="<?= h(url_for('/')) ?>" aria-label="<?= h($site_name) ?>" style="display:inline-flex;align-items:center;justify-content:center;width:48px;height:48px;">
          ...
          <?php else: ?>
            <svg width="44" height="44" viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="<?= h($site_name) ?>"><rect width="44" height="44" rx="8" fill="#111"/><text x="50%" y="54%" text-anchor="middle" font-family="system-ui,Segoe UI,Arial" font-size="18" fill="#fff">MK</text></svg>
          <?php endif; ?>
        </a>
